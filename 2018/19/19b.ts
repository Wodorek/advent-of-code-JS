...
const device = new OpcodeDevice([1, 0, 0, 0, 0, 0], bindTo);

let currentInst = device.registers[bindTo];

while (device.working) {
  ...
  registers.forEach((reg, idx) => {
    maxRegValue = Math.max(maxRegValue, reg);
  });
  currentInst = device.executeInstruction(...instructions[currentInst]);
  totalLoops++;
}

function calculateSum(num: any) {
  const arr: any = [...new Array(num + 1).keys()]
    .slice(1)
    .reduce((s: any, a: any) => s + (!(num % a) && a), 0);

  return arr;
}

console.log(calculateSum(maxRegValue));
