...
class Filesystem {
  ...
  current: Directory;
  availableSpace: number = 70000000;
  requiredSpace: number = 30000000;
  ...
  findBigEnough() {
    ...
    const emptySpace = this.availableSpace - this.root.size;
    ...
    while (queue.length > 0) {
      ...
      if (current.size + emptySpace >= this.requiredSpace) {
        matching.push(current.size);
      }
...
console.log(Math.min(...files.findBigEnough()));
