...
const cubeSize = 1 + 6 * 2;
const minimal = cubeSize * -1;

for (let q = minimal; q <= cubeSize; q++) {
...
function findNeighbors(position: string, cubesMap: CubesMap) {
  const [q, z, y, x] = position.split(',').map(Number);

  let totalBlack = 0;
  for (let h = q - 1; h <= q + 1; h++) {
    for (let i = z - 1; i <= z + 1; i++) {
      for (let j = y - 1; j <= y + 1; j++) {
        for (let k = x - 1; k <= x + 1; k++) {
          const neighborId = `${h},${i},${j},${k}`;
